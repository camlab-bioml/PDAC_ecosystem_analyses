...
```{r}
top_ha <- columnAnnotation(Validated.sig = column.annot$validated.sig.name)
```

```{r}
png("test11.png", width = 18, height = 10, units = "in", res = 300)
Heatmap(holder.sigprofile.mtx %>% scale(), 
        name = "Norm sig-loading",
        #left_annotation = row_ha, 
        #row_split = sigprofile.df$cohort,
        top_annotation = top_ha,
        column_split = (holder.sigprofile.df %>% select(!c(cohort, sample)) %>% names() %>% str_split(., pattern = " ", simplify = T))[,1])
dev.off()
```

```{r}
write_tsv(column.annot, file = "dis-val-sig-mapping.tsv")
```
